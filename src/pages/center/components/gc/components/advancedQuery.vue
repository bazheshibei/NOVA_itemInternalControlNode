...
export default {
  data() {
    return {
      selectObj: [
        ...
        { label: '工厂交期', value: 'plant_deliver_date' },
        { label: '工厂名称', value: 'provider_name' },
        { label: '工厂简称', value: 'short_name' }
      ],
      valueType: {
        item_name: 'text',
        item_season: 'text',
  ...
  methods: {
    ...
    _proving() {
      ...
      /* 保存有用数据 */
      this.$store.commit('saveData', { name: 'filter_data', obj: arr, module: 'Gc' })
    },
    /**
     * [增加条件]
     */
    ...
    reset() {
      ...
      this.$store.commit('saveData', { name: 'filter_data', obj: [], module: 'Gc' })
      /* 重置分页 */
     ...
     */
    handleClose() {
      this.$store.commit('saveData', { name: 'isDialog', obj: false, module: 'Gc' })
    },
    /**
     * [计算：表格高度]
     */
