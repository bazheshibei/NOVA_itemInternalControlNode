...
export default {
  ...
  computed: {
    ...mapState('Dh', ['pagenum', 'rownum', 'pageCount', 'item_id', 'item_gantt_id', 'item_gantt_detail_id', 'tableRow_1'])
  },
  methods: {
    /**
     * [排计划]
    ...
    plant() {
      ...
      /* 保存到本地缓存 */
      localStorage.setItem('sumittcpcGanttSummaryType', JSON.stringify({ type: 1 }))
      /* win 方法打开页面 */
    ...
    edit() {
      const that = this
      /* 提取ID */
      const { tableRow_1: { item_gantt_detail_id } } = this
      /* 保存到本地缓存 */
      const { item_id, item_gantt_id } = this
      localStorage.setItem('NOVA_reject', JSON.stringify({ item_id, item_gantt_id, item_gantt_detail_id, from: 'neikong' }))
      /* win 方法打开页面 */
     ...
     */
    advancedQuery() {
      this.$store.commit('saveData', { name: 'isDialog', obj: true, module: 'Dh' })
    },
    /**
     * [刷新]
    ...
    pageChange(name, val) {
      /** 保存数据 **/
      this.$store.commit('saveData', { module: 'Dh', name, obj: val })
      /** 发起请求 **/
      this._request()
    },
    /**
